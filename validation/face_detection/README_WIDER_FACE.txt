...
The validation scripts locate this directory automatically from the project structure. No machine-specific dataset path needs to be edited in the scripts.

Validation files
----------------
wider_face_inference.py
    Runs face detection on all images in the WIDER FACE validation split and writes the prediction files required by the evaluator.
...
--------------------------
Before generating a new qualitative result set, wider_face_qualitative.py removes only its own previous qualitative outputs:
